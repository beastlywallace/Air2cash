...
function createData(date, email, phonenumber, network, amountsent, amountreceived, status) {
  ...
  const transactionDetails = (row) => {
    navigate(`/dashboard/admin/transactions_details/${row.id}`)
  }
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {rowData
              ...
              .map((row) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.code} style={{cursor:"pointer"}} onClick={()=>transactionDetails(row)}>
                    {columns.map((column) => {
